...
// --- API Service (HEAD Branch functionality) ---
...
export interface ApiArtworksResponse {
  artworks: ApiArtwork[]
  total: number
  page: number
  limit: number
...
export interface ApiArtworkFilters {
  page?: number
  limit?: number
  category?: string
  creator?: string
...
export const artworkService = {
  async getArtworks(filters?: ApiArtworkFilters): Promise<ApiArtworksResponse> {
    try {
...
// --- React Query Hooks (origin/main functionality) ---
...
const buildDefaultProfile = (): UserProfile => ({
  ...
  stats: {
    created: MOCK_ARTWORKS.filter((a) => a.creator === DEFAULT_PROFILE_ADDRESS).length,
    collected: 4,
    favorites: 7,
    totalSales: '18.4 XLM',
...
export function useTrendingArtworks() {
  return useQuery({
    ...
    queryFn: async () => [...MOCK_ARTWORKS].sort((a, b) => ((b as Artwork).views ?? 0) - ((a as Artwork).views ?? 0)).slice(0, 6) as Artwork[],
  })
}
...
export function useUserArtworks(userAddress: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Artwork[]> => {
      if (!userAddress) {
        return MOCK_ARTWORKS.filter((a) => a.creator === DEFAULT_PROFILE_ADDRESS) as Artwork[]
      }

      const created = MOCK_ARTWORKS.filter((a) => a.creator === userAddress)
...
export function useUserCollection(userAddress: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<Artwork[]> => {
      ...
      const notOwnedByUser = MOCK_ARTWORKS.filter((a) => a.creator !== userAddress)
